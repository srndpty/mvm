...
$input=[ordered]@{schema='mvm-p2-r4-visible-input-1';case_id='contract';sync_interval=1
...
switch($Case){
    'NegativeOriginalCallback'{$input.callbacks[0].original_callback_qpc=150}
    'NegativeSyntheticDelay'{$input.callbacks[0].synthetic_delay_ticks=100}
    'NegativeExpectedClass'{$input.expected_solution_class='UNIQUE'}
    'NegativeDisplayed'{$input.callbacks[0].displayed=@([ordered]@{qpc=100})}
}
