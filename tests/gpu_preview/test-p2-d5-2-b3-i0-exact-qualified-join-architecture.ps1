...
Require $queue 'activeReservation_\s*=\s*\{\+\+reservationSerial_,[\s\S]+requiredIntentOrdinals_' 'primary reservation identityがrequired queue由来ではありません'
Require $scheduler 'queueDecision\.reservation\.reservationId' 'required queue reservationがscheduler decisionへexact transportされません'
