...
// フレーム固有マーカーの読み取り
// --------------------------------------------------------------------------
// 実装は src/core/mvm_marker.h に一本化した。
// Phase 1 の GPU preview スパイクが同じ判定を使うため、
// ハーネス専用ヘッダから出した。ここでは名前だけ引き込む。
// 仕様: docs/research/test-media-format.md

using mvm::marker::kCellCount;
using mvm::marker::kCellSize;
using mvm::marker::kDataCells;
using mvm::marker::MarkerRead;
using mvm::marker::readMarker;
using mvm::marker::readMarkerAuto;
using mvm::marker::readMarkerScaled;
using mvm::marker::readMarkerWithCellSize;
