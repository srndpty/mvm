...
| 経路 | V1 | V2 | 点数 | seek p95 | 観測 max | M5 |
| --- | --- | --- | --- | --- | --- | --- |
| 4K original | 4K | 1080p HEVC | 300 | 888.2ms | 2343.3ms | 不合格 |
| V1-only proxy (partial) | 540p | **1080p HEVC** | 300 | 452.4〜460.5ms | 574.6ms | 不合格 |
| all-video proxy (暫定) | 540p | 540p | 300 | 81.7ms | 285.6ms | (暫定合格) |
| **all-video proxy (最終)** | 540p | **540p** | **1000** | **82.7ms** | **2998.0ms** | **不合格** |

**[事実] V2 を proxy 化すると p95 は基準内に入る。**
V1 だけでは p95 452ms、両方 proxy 化して p95 82.7ms である。
partial proxy の測定を「proxy の実力」として読んではいけない。

## [事実] 1000 点の最終確認で M5 は不合格になった

300 点での「合格」は暫定であり、**1000 点 x 3 回で覆った。**

計測条件: `random 1000`、`seed 20260804` 固定、3 回、run ごとに別プロセス。

| run | mismatch | p50 | p95 | max |
| --- | --- | --- | --- | --- |
| 1 | 0 | 65.2ms | 90.0ms | **2998.0ms** |
| 2 | 0 | 57.8ms | 81.7ms | 225.1ms |
| 3 | 0 | 60.9ms | 82.7ms | 230.2ms |
| 中央値 | **0** | 60.9ms | **82.7ms** | 観測 max **2998.0ms** |

基準は mismatch 0 / p95 <= 150ms / **max <= 400ms**。
mismatch と p95 は満たすが、**観測 max が 2998ms で 7.5 倍超過している。**
よって **M5 は不合格**である。

### 超過しているのは cold の 1 回目だけである

3 回 x 1014 点 (固定点 14 + random 1000) = 3042 サンプルのうち、
400ms を超えたのは **1 件だけ**だった。
それは各 run の **index 0 (`cold=1` / `category=first`)**、
つまりグラフ構築後の最初の seek である。

件数の内訳 (`build/<preset>/seek-all-1000/*.json` の生 JSON から集計):
total 3042 = cold 3 + warm 3039 = random 3000 + fixed 42。

| 区分 | 件数 | p50 | p95 | max |
| --- | --- | --- | --- | --- |
| warm (cold を除く全 run) | 3039 | 61.1ms | 85.2ms | **132.0ms** |
| cold (各 run の 1 回目) | 3 | — | — | 225.1 / 230.3 / **2998.0** ms |

**[事実] warm の 3039 サンプルは全て 132ms 以下**であり、基準内に十分収まる。
**[事実] cold の 1 回目は 225ms から 2998ms まで 13 倍ばらつく。**

**[推測] 原因は特定していない。** グラフ構築直後の初回デコードであり、
ファイルキャッシュの状態、producer の初期化、
他プロセスとの I/O 競合のいずれもありうる。切り分けは行っていない。

### 判定を緩めない

**「cold を除けば合格」という書き方で M5 を合格にはしない。**
基準は「観測した max」であり、cold の 1 回目も利用者が実際に待つ時間である。
編集開始直後の最初の seek で 3 秒待たされるのは、
数値上どう区分しようと実際に遅い。

ただし **M5 の合否は Phase 0 の結論を左右しない。**
判定を決めているのは M7 / M8 (連続再生の fps) であり、
M5 が仮に合格でも結論は変わらない
