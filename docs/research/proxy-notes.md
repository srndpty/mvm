...
音声 2 本・文字レイヤが残っており、
**[推測] コストはそれらを含む合成全体に広がっている**と考えられる。
V1 の解像度だけが要因ではない (合成が唯一の律速だと証明したわけではない)。
...
4K を 540p に落としても、**1080p native と同程度から改善しなかった**。
**[推測]** 解像度ではなく合成側にコストが集中していることを示唆するが、
**「合成そのものが律速であると証明した」とまでは書かない** (MLT 内部の切り分けは
行っておらず、原因は特定していない)。
...
### [事実] ある時間帯だけ測定値が約 2.5 倍になった (原因は未特定)

2026-08-06 の測定中、**ある時間帯だけ測定値が全体的に約 2.5 倍になった。**
...
**[推測] ホスト側の要因かもしれない。** CPU の周波数制御、Windows の
効率化モード (EcoQoS)、常駐プロセスの負荷などが候補として考えられるが、
**いずれも確かめておらず、原因は特定していない。**
MLT 側の要因である可能性も排除できていない。
**「ホスト側の要因と特定した」とは書かない。**

### 対処

- **run 間の最大/最小が 1.25 倍を超えたケースは `SpreadWithinThreshold=いいえ`
  として記録し、判断に使わない。** 中央値は「たまたま多かった側」を指すだけなので、
  中央値を出す前に弾く。閾値内で測れたケースまで捨てないよう、実行は続ける
- **A > B のような想定外の性能順序は `UnexpectedOrdering` として警告し、
  JSON に記録するだけ**にした。**hard failure にはしない。**
  性能の順序は correctness invariant ではない。
  「A は B より仕事が多いから遅いはず」は妥当な期待だが、
  それが破れたことをもって実装の誤りとは言えない
...
**正式な M7 / M8 プロトコルの 9 回** (前回セッションの 60 秒 x 3、当日の直接再測定 x 3、
呼び出し方比較 x 3) では 18.74〜19.83 fps に収まった。
一方、diagnostic な ablation の実行では性能状態が大きく変動し、
同じ論理構成でも 48.71 fps を観測したことがある (上記「機械そのものが一時的に
2.5 倍速い状態に入る」節)。**formal protocol と diagnostic protocol の結果は食い違う。**
したがって、この 9 回を「M8 は常に安定」と一般化しては書かない。
formal protocol の 9 回に限った事実として扱う。

---

**以下の ablation fps 表は参考値であり、再現不能・判定非依存である。**
この履歴値をここ (proxy-notes.md) にのみ残し、判定書と ADR には載せない。
基準は B (all-video proxy)。
