...
| **M5** | 1000 点 x 3: p95 **82.7ms** / 観測 max **2998.0ms** (基準 150 / 400) | **不合格** |
...
### all-video proxy は seek の p95 を救ったが、M5 も M8 も救わなかった

**[事実] proxy を全 video へ広げると seek の p95 は基準内に入る。**
p95 888ms (native) -> 452ms (V1-only) -> **82.7ms (all-video)**。

**[事実] しかし 1000 点 x 3 回の最終確認で M5 は不合格だった。**
300 点では観測 max 285.6ms で暫定合格だったが、
1000 点では **観測 max 2998.0ms** で基準 400ms を 7.5 倍超過した。
超過しているのは各 run の 1 回目 (cold) の 1 サンプルだけで、
warm の 3039 サンプルは p95 85.2ms / max 132.0ms に収まる。
全サンプルは 3 回合計 3042 (cold 3 + warm 3039 = random 3000 + fixed 42)。
**「cold を除けば合格」という書き方はしない**
(`docs/research/seek-scrub-notes.md`)。

**[推測] cold の 1 回目が 225ms〜2998ms とばらつく原因は特定していない。**

**[事実] しかし連続再生の fps は救えない。**
19.67 fps は 1080p native (19.85) をわずかに下回る。
**素材の解像度を下げても 20 fps 前後から改善しなかった** (これが物理上限だと
証明したわけではない。原因は特定していない)。

したがって「常時 proxy を使う条件付き採用」は成立しない。
その条件下でも M7 / M8 の fps が満たせないからである。

### 回避策の探索 (ablation) — 定量的根拠には採用しなかった

「何を外せば速くなるか」を preview で測る ablation を行った。
**[注意] ablation は run 間の変動が大きく、個別最適化の定量的根拠には採用しなかった。**
同じ論理構成でも実行ごとに大きくばらつき、別の実行では基準構成が **48.71 fps** を
記録したこともある (それでも基準 50 未満)。変動の原因は特定していない。

正式な M7 / M8 プロトコル (60 秒 x 3 を含む計 9 回) では 18.74〜19.83 fps に収まった。
一方 diagnostic な ablation の結果とは食い違う。
**したがって不採用の理由は、性能不足と再現性不足の両方である。**

ケース別の具体的な fps 値は
[research/proxy-notes.md](research/proxy-notes.md) にのみ残す
(**参考値・再現不能・判定非依存**)。この所見からは再現できなかった fps 表を除いた。
...
- all-video proxy でも 20 fps 前後から改善しなかった (基準 50 に届かない)
- 性能状態が大きく変動し、再現しない (formal と diagnostic の結果が食い違う)
- 単独の低コスト回避策で 50fps へ届く根拠が無い
