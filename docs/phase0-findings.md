...
**[注意] 以下の ablation の値は参考値であり、判定の根拠に使っていない。**
2026-08-06 の再測定で、同じ scenario が run ごとに 2.5 倍ばらつく現象が出た
(ホスト側の要因。詳細は `docs/research/proxy-notes.md`)。
**判定を決めた M7 / M8 はこの表に依存しない。**
M8 の all-video proxy は独立に 9 回測って 18.74〜19.83 fps に収まっており再現している。

ablation (preview × 3 回) の結果:

| 除去したもの | fps | 製品で外せるか |
| --- | --- | --- |
| なし (all-video proxy 基準) | 21.53 | — |
| qtext を静的 RGBA overlay の代用物へ (ケース F) | **31.1** | 未確認 (下記) |
| V2 / affine 合成を除去 | **35.84** | 外せない (PiP は必須) |
| qtext を除去 | 33.65 | 外せない (文字は必須) |
| V2 proxy を 640x360 へ | 24.47 | 外せる |
| 音声 2 トラックを除去 | 22.45 | 外せない。効果も小さい |

**[注意] ケース F は「文字を事前描画した画像」ではない。**
使ったのは既存の `png_alpha.png` で文字は入っていない。
測ったのは「qtext の毎フレーム描画を、同じ位置・同じ大きさの
**静的 RGBA overlay 1 枚**に置き換えた場合の上限」だけである。
実際の文字画像で同じ fps になる保証は無く、視覚的な同等性も確認していない。

**[事実] 製品で使えると確認済みの単独の回避策は最良で 24.47 fps。**
**代用物での上限 (F) を含めても 31.1 fps。**
